...
export const sharpImages = async () => {
  ...
  const glob = new Glob(`src/assets/**/*.{${SHARP_FILE_TYPE_LIST.join(',')}}`)
  ...
  const sharpBeforeSize = sharpedImageList.reduce((ac, v) => ac + v.beforeSize, 0)
  const sharpAfterSize = sharpedImageList.reduce((ac, v) => ac + v.afterSize, 0)
  const metrics = {
    ...
    savedPercent: sharpBeforeSize > 0 ? +((1 - sharpAfterSize / sharpBeforeSize) * 100).toFixed(2) : 0
  }
